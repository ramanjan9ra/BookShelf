...
@section('content')
<div class="mb-6">
    <a href="{{ route('books.show', $book) }}" class="inline-flex items-center text-gray-600 hover:text-indigo-600 mb-4 transition duration-150 ease-in-out">
...
<div class="bg-white shadow-lg rounded-lg p-8 border border-gray-200 transition-all duration-300 hover:shadow-xl">
    <form action="{{ route('books.update', $book) }}" method="POST">
            ...
            <div>
                ...
                <div class="relative rounded-md shadow-sm">
                    ...
                    <textarea name="description" id="description" rows="12"
                         class="w-full pl-10 pr-3 py-2.5 rounded-md border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200 @error('description') border-red-300 @enderror">{{ old('description', $book->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 flex items-center">
                            <i class="fas fa-exclamation-circle mr-1"></i>
                            {{ $message }}
        ...
        <div class="mt-8 pt-6 border-t border-gray-200 flex justify-end">
            <a href="{{ route('books.show', $book) }}" class="bg-white py-2.5 px-5 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200 mr-4 flex items-center">
